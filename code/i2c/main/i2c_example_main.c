...
#define TSL4531_CMD_CONTROL_REG            0x80    /*!< Command to set measure mode */
#define TSL4531_CMD_NORMAL_OPERATION       0x03    /*!< Command to set measure mode */
#define TSL4531_CMD_MEASURE_LIGHT          0x84    /*!< Command to set measure mode */
...
static esp_err_t TSL4531_measure_light_temp(i2c_port_t i2c_num, uint8_t* data_h, uint8_t* data_l)
{
    i2c_cmd_handle_t cmd = i2c_cmd_link_create();
    i2c_master_start(cmd);
...
static esp_err_t TSL4531_init(i2c_port_t i2c_num)
{
    i2c_cmd_handle_t cmd = i2c_cmd_link_create();
    i2c_master_start(cmd);
    i2c_master_write_byte(cmd, TSL4531_SENSOR_ADDR << 1 | WRITE_BIT, ACK_CHECK_EN);
    i2c_master_write_byte(cmd, TSL4531_CMD_CONTROL_REG, ACK_CHECK_EN);
    i2c_master_write_byte(cmd, TSL4531_CMD_NORMAL_OPERATION, ACK_CHECK_EN);
    i2c_master_stop(cmd);
...
static void i2c_test_task_temp(void* arg)
{
    int i = 0;
    int ret;
...
static void i2c_test_task(void* arg)
{
    ...
    uint32_t task_idx = (uint32_t) arg;
    uint8_t sensor_data_h, sensor_data_l;
    bool TSL4531_initialized = false;
    ...
    while (1) {
        ...
        //---------------------------------------------------//
        if (!TSL4531_initialized) {
          ret = TSL4531_init(I2C_EXAMPLE_MASTER_NUM);
          if (ret == ESP_OK) {
              printf("TSL4531_init...hopefully\n");
              TSL4531_initialized = true;
          } else {
              printf("SI7020_measure_hum No ack, sensor not connected\n");
          }
          vTaskDelay(( CLIMATE_DELAY_TIME * ( task_idx + 1 ) ) / portTICK_RATE_MS);
...
void app_main()
{
    ...
    xTaskCreate(i2c_test_task, "i2c_test_task_0", 1024 * 2, (void* ) 1, 10, NULL);
