...
void set_brightness(int brightness) {
  ...
  if (!neutral_present) {
  if (triac_delay<0.0040 && !neutral_present) {
...
void app_main() {
  ...
  //example_tg0_timer_init(TIMER_1, TEST_WITH_RELOAD,    triac_delay);
  //example_tg0_timer_init(TIMER_0, TEST_WITHOUT_RELOAD, sim_zerocross_delay);
  xTaskCreate(timer_example_evt_task, "timer_evt_task", 2048, NULL, 10, NULL);
  //gpio_evt_queue = xQueueCreate(10, sizeof(uint32_t));
  gpio_init();
  while(1) {
    fade_brightness(0,255,5000);
    fade_brightness(255,0,5000);
    //gpio_set_level(TRIAC_IO, 0);
