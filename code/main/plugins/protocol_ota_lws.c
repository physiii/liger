...
#define CONFIG_LWS_OTA_SERVER_FQDN "y"
...
struct per_session_data__ota {
	struct per_session_data__ota *next;
...
struct per_vhost_data__ota {
	...
	struct per_session_data__ota *live_pss_list;
...
static void
scan_start(struct per_vhost_data__ota *vhd)
{
	int n;
...
{
	struct per_vhost_data__ota *vhd = pvTimerGetTimerID(t);
...
static int
client_connection(struct per_vhost_data__ota *vhd, const char *file)
{
	...
	//snprintf(path, sizeof(path) - 1, "downloads/liger.bin" CONFIG_LWS_MODEL_NAME "/%s", file);
...
scan_finished(uint16_t count, wifi_ap_record_t *recs, void *v)
{
	struct per_vhost_data__ota *vhd = v;
	struct per_session_data__ota *p = vhd->live_pss_list;
	       ...
	       char *buf, int len, enum lws_spa_fileupload_states state)
{
	struct per_session_data__ota *pss =
			(struct per_session_data__ota *)data;
	int n;
