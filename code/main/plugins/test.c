...
void adc1task(void* args)
{
  ...
  int n, m;
  while(1){
      /*for (int i = 0; i < adc1_get_voltage(MIC_CHANNEL); i++) {
        printf(" ");
        i=i+10;
      }
      printf("<>\n");*/
      n = lws_snprintf((char *)voltage, sizeof(buf) - LWS_PRE, "%d", 123);
      //m = lws_write(wsi, voltage, n, LWS_WRITE_TEXT);
      /*if (m < n) {
        lwsl_err("ERROR %d writing to di socket\n", n);
        return -1;
      }*/
      printf("The adc1 value:%s\n",voltage);
			...
			void *user, void *in, size_t len)
{
	...
	case LWS_CALLBACK_SERVER_WRITEABLE:
		n = lws_snprintf((char *)p, sizeof(buf) - LWS_PRE, "%d", pss->value++);
	...
	case LWS_CALLBACK_RECEIVE:
		...
		if (strcmp((const char *)in, "closeme\n") == 0) {
			lwsl_notice("microphone: closing as requested\n");
