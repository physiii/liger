...
static void nec_rx_init()
{
    ...
    rmt_config(&rmt_rx);
    rmt_driver_install(rmt_rx.channel, 5000, 0);
}
        ...
        if(item) {
            ...
            while(1) {
                ...
                if(res > 0) {
                    ...
                    ESP_LOGI(NEC_TAG, "BREAKING LOOP -- RMT RCV --- addr: 0x%04x cmd: 0x%04x", rmt_addr, rmt_cmd);
                    break;
                }
            }
            ESP_LOGI(NEC_TAG, "bottom of rb loop -- RMT RCV --- addr: 0x%04x cmd: 0x%04x", rmt_addr, rmt_cmd);
